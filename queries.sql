INSERT INTO categories (name, img_background, LotsId) VALUES("Доски и лыжи", "promo__item--boards", "1"),
...
INSERT INTO lots (title, description, img_link, price, date_of_creation, date_of_completion,
...
SELECT name, img_background FROM categories ORDER BY name ASC;
...
SELECT l.title, l.price, l.date_of_creation, l.img_link, c.name, MAX(b.sum) AS "Max_price"
FROM lots l
JOIN categories c ON c.lotsId = l.id
LEFT OUTER JOIN bets b ON b.LotsId = l.id
WHERE date_of_completion > NOW() GROUP BY l.title, l.price, l.date_of_creation, l.img_link, c.name;
